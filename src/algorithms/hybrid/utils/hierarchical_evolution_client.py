...
from algorithms.hybrid.utils.root_client import RootClient
...
class HierarchicalEvolutionClient:
    ...
    def __init__(
        ...
        self._rootClient: RootClient = RootClient()
    ...
    def evolve(self) -> None:
        ...
        root: int = self._rootClient.getRootIndividual()
        TUI.appendToSolverLog(f"Root is: {root}.")
        self._metaPopSize, self._genesisPopSize = self._computeMetaAndGenesisPopSize(root)
        ...
        while (
            ...
            while len(qualifiedIndividuals) < self._minQualInd and shouldMetaGenContinue and gen <= self._maxGen:
                ...
                while (
                    ...
                    newRoot: int = self._rootClient.getRootIndividual()

                    if newRoot != root:
                        didRootChange = True
                        TUI.appendToSolverLog(
                            f"Root has changed from {root} to {newRoot}."
                        )
            ...
            if not didRootChange:
                ...
                if isCurrentRootDominant:
                    ...
                    self._rootClient.selectRootNeighbour(
                        currentRootIndividual, rootRadius
                    )
                else:
                ...
                currentRootIndividual = root
            newRoot: int = self._rootClient.getRootIndividual()
            TUI.appendToSolverLog(f"New root: {newRoot}. Previous root: {root}.")
            self._metaPopSize, self._genesisPopSize = self._computeMetaAndGenesisPopSize(newRoot)
            TUI.appendToSolverLog(f"Meta pop size: {self._metaPopSize}, Genesis pop size: {self._genesisPopSize}")
